...
export const useProjectStore = create<ProjectStore>()(
  persist(
    (set, get) => ({
      ...
      updateFurnitureMoving: (projectId, includeFurnitureMoving) => {
        set((state) => ({
          projects: state.projects.map((p) =>
            p.id === projectId
              ? {
      ...
      setProposalSent: (projectId, sent) => {
        set((state) => ({
          projects: state.projects.map((p) =>
            p.id === projectId
              ? {
      ...
      addRoom: (projectId, floorNumber) => {
        ...
        const newRoom: Room = {
          ...
          includeDoorlessClosetInteriorInQuote: globalDefaults.paintClosetInteriors,
        ...
        set((state) => ({
          projects: state.projects.map((p) =>
            p.id === projectId
              ? {
                  ...p,
                  bathrooms: [...(p.bathrooms || []), newBathroom],
                  updatedAt: Date.now(),
                }
              : p
          ),
      ...
      addIrregularRoom: (projectId) => {
        ...
        const newIrregularRoom: IrregularRoom = {
          ...
          hasCloset: false,
          doorlessClosets: 0,
          includeClosetInteriorInQuote: project?.projectIncludeClosetInteriorInQuote ?? true,
