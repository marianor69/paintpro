...
import { formatCurrency, getClosetInteriorMetrics, safeNumber } from "../utils/calculations";
...
export default function IrregularRoomEditorScreen({ route, navigation }: Props) {
  ...
  const [paintWindows, setPaintWindows] = useState(!isNew ? irregularRoom?.paintWindows ?? true : true);
              ...
              <Card style={{ marginBottom: Spacing.md }}>
                ...
                {!hasPricingData ? (
                  ...
                  <View style={{ flexDirection: "row", gap: Spacing.sm }}>
                    <View style={{ flex: 1.05, backgroundColor: Colors.backgroundWarmGray, borderRadius: 8, padding: Spacing.md }}>
                      ...
                      {hasCloset && (
                        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                          ...
                          <Text style={{ fontSize: 13, color: Colors.darkCharcoal }}>
                            {singleClosetCount + doubleClosetCount}
                          </Text>
                        </View>
                      )}
                    </View>

                    <View style={{ flex: 0.95, backgroundColor: Colors.primaryBlueLight, borderRadius: 8, padding: Spacing.md }}>
                      ...
                      {wallGallonsTotal > 0 && (
                        <Text style={{ fontSize: 13, color: Colors.darkCharcoal, textAlign: "right", marginBottom: Spacing.xs }}>
                          Wall: {wallGallonsTotal.toFixed(2)} gal
                        </Text>
                      )}
                      {ceilingGallonsTotal > 0 && (
                        <Text style={{ fontSize: 13, color: Colors.darkCharcoal, textAlign: "right", marginBottom: Spacing.xs }}>
                          Ceiling: {ceilingGallonsTotal.toFixed(2)} gal
                        </Text>
                      )}
                      {trimGallonsTotal > 0 && (
                        <Text style={{ fontSize: 13, color: Colors.darkCharcoal, textAlign: "right", marginBottom: Spacing.xs }}>
                          Trim: {trimGallonsTotal.toFixed(2)} gal
                        </Text>
                      )}
                      {paintDoors && doorGallons > 0 && (
                        <Text style={{ fontSize: 13, color: Colors.darkCharcoal, textAlign: "right" }}>
                          Doors: {doorGallons.toFixed(2)} gal
                        </Text>
                      )}
                    </View>
                  </View>
